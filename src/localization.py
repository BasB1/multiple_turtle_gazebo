...
#         
#    def getTransformData(self):                                                 # Updates the most rescent localized position of the external robot.
#        (self.trans, self.rot) = self.listener.lookupTransform(self.tf_prefix + '/odom', self.tf_prefix + '/robot_pos_1', rospy.Time(0))
#        rospy.loginfo((self.trans[0] * self.trans[0] + self.trans[1] * self.trans[1]) ** 0.5)
#
#    def odomData(self):                                                         # Updates the received odom data for the transform class.
...
class Localize(object):                                                         # The Localiztion class holds all functions related to Pozyx measurements and localization calculations.
    def __init__(self, robot_list, tag_pos, robot_number, alpha, noise, R, link_to_robot, tf_prefix):
...
#            pass
    ...
    def getDistances(self):
        ...
        self.distance_1 = pzx.doRanging(self.A, self.C)
        self.distance_3 = pzx.doRanging(self.A, self.D)
        self.distance_2 = pzx.doRanging(self.B, self.C)        
        self.distance_4 = pzx.doRanging(self.B, self.D)
                
        self.f1.update(self.distance_1)                                         # Update KF with measurement
        self.f2.update(self.distance_2)
        self.f3.update(self.distance_3)
        self.f4.update(self.distance_4)  
    ...
    def triangulationLocal(self):
        ...
        D_left = abs((T + r_0 + r_1) * (T + r_0 - r_1) * (T - r_0  + r_1) * (-T + r_0 + r_1))
        D_right = abs((T + r_2 + r_3) * (T + r_2 - r_3) * (T - r_2  + r_3) * (-T + r_2 + r_3))
        
        O_left = (D_left ** 0.5) / 4
        O_right = (D_right ** 0.5) / 4

        LEFT_x_1 = ((a + c) / 2) + (((c - a) * (r_0 ** 2 - r_1 ** 2)) / (2 * T ** 2)) + 2 * ((b - d) / (T ** 2)) * O_left
        LEFT_x_2 = ((a + c) / 2) + (((c - a) * (r_0 ** 2 - r_1 ** 2)) / (2 * T ** 2)) - 2 * ((b - d) / (T ** 2)) * O_left
        LEFT_y_1 = ((b + d) / 2) + (((d - b) * (r_0 ** 2 - r_1 ** 2)) / (2 * T ** 2)) + 2 * ((a - c) / (T ** 2)) * O_left
        LEFT_y_2 = ((b + d) / 2) + (((d - b) * (r_0 ** 2 - r_1 ** 2)) / (2 * T ** 2)) - 2 * ((a - c) / (T ** 2)) * O_left

        RIGHT_x_1 = ((a + c) / 2) + (((c - a) * (r_2 ** 2 - r_3 ** 2)) / (2 * T ** 2)) + 2 * ((b - d) / (T ** 2)) * O_right
        RIGHT_x_2 = ((a + c) / 2) + (((c - a) * (r_2 ** 2 - r_3 ** 2)) / (2 * T ** 2)) - 2 * ((b - d) / (T ** 2)) * O_right
        ...
        try:
            ...
            self.br.sendTransform((LEFT_x_1, LEFT_y_1, 0),                          # Create the transforms for vizualization purpose
                         ...
                         self.tf_prefix + self.link_to_robot)
...
#        return 0
#    
#    def odomData(self, data):                                                   # Update the odom data for the communication class.
#        self.odom_data = data
#        
#    def txData(self):
#        return 0
...
if __name__ == "__main__":
    ...
    loc = Localize(robot_list, tag_pos, robot_number, alpha, noise, R, link_to_robot, tf_prefix)
#    com = Communicate(robot_number, robot_list)
